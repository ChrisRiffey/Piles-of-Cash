...
public class Teleport : MonoBehaviour 
	...
	void Update () 
		...
		if(Input.GetButtonDown("Fire1")) // if fire button pressed
			{
			Ray tRay; //initialize ray
			RaycastHit hit; //initialize hit 
			tRay = Camera.main.ScreenPointToRay(Input.mousePosition); // raycast from screen point
			if (Physics.Raycast(tRay, out hit, 100.0f)) // if raycast is hitting something
				{
				if ((hit.collider.tag == "floor") && (warping == false)) // raycast is hitting a floor object
					{
					startTime = Time.time; //record the time
					warpTarget = hit.point + new Vector3 (0, 1.3f, 0); // specify the warp target with offset for height
